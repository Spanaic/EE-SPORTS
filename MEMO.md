...
## firestoreのドキュメントを削除すると、undefineドキュメントが作成される問題

問題点(2019/1/9時点)

1. メッセージの参照の仕方がuser.idベースになっている可能性がある。(room単位ではなくて)
2. roomが削除出来る場合、もしくはuser.idやroom.idがかぶる場合、前のメッセージが表示される可能性がある。
